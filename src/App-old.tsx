...
export default function App() {
  ...
  const [updateMessage, setUpdateMessage] = useState<string>(
    "Bibliothèque hors ligne prête à l'emploi.",
  );
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (update) {
        console.log(`Mise à jour trouvée: ${update.version} du ${update.date}`);

        // On lance le téléchargement
        await update.downloadAndInstall();

        // On redémarre l'app pour appliquer l'APK
        await relaunch();
      } else {
          ...
          <div className="relative overflow-hidden rounded-[28px] bg-gradient-to-r from-purple-500 via-fuchsia-500 to-pink-500 p-5 text-white shadow-lg shadow-purple-200">
            ...
            <div className="relative z-10">
              ...
              <p className="mt-1 max-w-[85%] text-sm text-white/80">
                Your app stays offline first, but you can fetch a new JSON when
                needed.
              </p>
              <button
                type="button"
        ...
        <div className="sticky top-0 z-10 bg-gradient-to-r from-purple-500 to-indigo-600 px-6 pb-6 pt-12 text-white shadow-md">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {selectedSong.categories.map((category) => (
              <span
                key={category}
                className="rounded-full bg-white/25 px-2.5 py-1 text-xs backdrop-blur-sm"
              >
                {category}
              </span>
            ))}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 pb-10">
          <div className="space-y-8">
            {selectedSong.parts.map((part) => (
              <div
                key={part.id}
                className="relative rounded-[28px] border border-slate-100/60 bg-white p-6 shadow-sm"
              >
                <span
                ...
                <p className="pt-2 text-lg leading-relaxed text-slate-800 whitespace-pre-line">
                  {part.content}
                </p>
              </div>
            ))}
